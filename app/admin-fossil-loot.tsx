...
type BugRarity = FossilLootEntry['rarity'];

export default function AdminFossilLootScreen() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const { theme } = useTheme();
  const { colors, typography, shadows } = theme;
  const [entries, setEntries] = useState<FossilLootEntry[]>([]);
  ...
  const load = useCallback(async () => {
    try {
      ...
      Alert.alert('Error', err.message ?? 'Failed to load fossil loot');
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  useEffect(() => {
    ...
    const save = async () => {
      setSaving(true);
      try {
        const res = await api.updateFossilLoot(entries);
        skipNextSaveRef.current = true;
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: FossilLootEntry; index: number }) => {
    ...
    <GradientBackground bubbleCount={2}>
      <View style={s.topSection}>
        <View style={[s.header, { paddingTop: insets.top + spacing.sm }]}>
          ...
          <Text style={styles.headerTitle}>Fossil Loot</Text>
          ...
          {loading ? (
            ...
            <>
              ...
              {entries.length === 0 && !saving ? (
                <Text style={styles.emptyText}>No items in loot pool. Add items to drop when fossils are dug up.</Text>
              ) : (
                <>
                  {saving && (
