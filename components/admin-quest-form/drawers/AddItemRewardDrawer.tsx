/**
 * Drawer for adding/editing an item reward.
 */
...
import type { ItemReward } from '../types';
import { useTheme } from '@/store/ThemeProvider';
import { createThemedStyles } from '../styles';

interface AddItemRewardDrawerProps {
  items: SearchableItem[];
  onSave: (item: ItemReward, editIndex?: number) => void;
}

export interface AddItemRewardDrawerRef {
  open: (initial?: ItemReward | null, editIndex?: number) => void;
}

export const AddItemRewardDrawer = forwardRef<AddItemRewardDrawerRef, AddItemRewardDrawerProps>(
  function AddItemRewardDrawer({ items, onSave }, ref) {
    const drawerRef = useRef<AppDrawerRef>(null);
    const [state, setState] = React.useState<ItemReward>({ itemType: '', qty: '1' });
    ...
    useImperativeHandle(ref, () => ({
      open: (init?: ItemReward | null, editIndex?: number) => {
    ...
    return (
      <AppDrawer ref={drawerRef} title="Item Reward" snapPoints={['90%']}>
        <View style={{ gap: 16 }}>
          <Field label="Item">
